...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zergi_i_travel_U_01\\travel.
    ...
    MODIFY ENTITIES OF zergi_i_travel_U_01
        ENTITY travel
    ...
    SELECT * FROM zergi_i_travel_U_01 WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zergi_i_travel_U_01' )
                                  ( i_for_entity = 'zsergi_i_booking_U_01' ) )
                                ).
  ENDMETHOD.
